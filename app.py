...
pages = ["🏠 Home Overview", "📋 Detailed Forecasts", "📈 Plotly Charts", "🗺️ Weather Map", "🔮 AI Predictions"]
...
# ----------------- PAGE 5: AI TEMPERATURE PREDICTION -----------------
elif selected_page == "🔮 AI Predictions":
    st.markdown(f"<h1 class='dashboard-title'>AI Predictions: {selected_loc}</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color:#94A3B8; margin-bottom:2rem;'>Evaluate temperature predictions generated by the RandomForestRegressor model.</p>", unsafe_allow_html=True)
    
    try:
        # Generate predictions dynamically
        pred_results = predict_temperatures()
        df_pred = pd.DataFrame(pred_results['predictions'])
        df_pred['forecast_time'] = pd.to_datetime(df_pred['forecast_time'])
        df_pred = df_pred.sort_values(by='forecast_time')
        
        # Filter predictions for the selected location
        df_loc_pred = df_pred[df_pred['location'] == selected_loc].copy()
        
        if df_loc_pred.empty:
            st.warning("No predictions found for this location.")
        else:
            # Model metrics cards
            m1, m2, m3 = st.columns(3)
            render_kpi(m1, "Model MAE", f"{pred_results['mae']:.4f}", "°C", "Mean Absolute Error")
            render_kpi(m2, "Model RMSE", f"{pred_results['rmse']:.4f}", "°C", "Root Mean Square Error")
            render_kpi(m3, "R² Determination Coeff", f"{pred_results['r2']*100:.1f}", "%", "Variance explained by the model")
            ...
            fig_compare.add_trace(go.Scatter(
                x=df_loc_pred['forecast_time'], y=df_loc_pred['actual_temperature'],
                name='CWA Forecast Target',
                line=dict(color='#38BDF8', width=3),
                mode='lines+markers'
            ))
            fig_compare.add_trace(go.Scatter(
                x=df_loc_pred['forecast_time'], y=df_loc_pred['predicted_temperature'],
            ...
            fig_compare.update_layout(
                title='7-Day Temperature: Actual CWA vs AI Prediction',
                xaxis_title='Time',
            ...
            col_scatter, col_residuals = st.columns(2)
            with col_scatter:
                # Scatter Plot
                fig_scatter = px.scatter(
                    df_loc_pred, x='actual_temperature', y='predicted_temperature',
                    trendline="ols",
                    title='Scatter Plot: Actual vs Predicted Temperature',
                    labels={'actual_temperature': 'Actual Temperature (°C)', 'predicted_temperature': 'Predicted Temperature (°C)'},
                    template='plotly_dark'
                )
            ...
            with col_residuals:
                # Residuals Bar Chart
                df_loc_pred['residual'] = df_loc_pred['actual_temperature'] - df_loc_pred['predicted_temperature']
                fig_residuals = px.bar(
                    df_loc_pred, x='forecast_time', y='residual',
                    title='Residual Plot (Actual - Predicted Offset)',
                    labels={'residual': 'Residual Error (°C)', 'forecast_time': 'Time'},
                    template='plotly_dark'
                )
                fig_residuals.update_traces(marker_color='#F43F5E', opacity=0.85)
        ...
        st.error(f"Failed to generate predictions. Make sure the model has been trained. Error: {e}")
